...
Agent 查源码路径文件定位代码；知识图暂不支持，源码定位走 paths.md 章节路径 + 直接 Read 源码。
...
- `范围：` — `暂不实施代码修改（仅步骤1-3）`/`全流程`，不填则到步骤4弹问
...
> **首次使用须填源码根路径**：各 `platforms/*_paths.md` 顶部的「源码根路径（GVM Android 侧）」「源码根路径（PVM ... 侧）」当前是占位 `<待填：…>`。请填入你本地对应的源码树根目录——paths.md 表内是相对路径，按侧拼对应根路径才能定位到文件。
